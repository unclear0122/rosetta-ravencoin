...
// specified address.
func PayToAddrScript(addr ravenutil.Address) ([]byte, error) {
	const nilAddrErrStr = "unable to generate payment script for nil address"
	switch addr := addr.(type) {
	case *ravenutil.AddressPubKeyHash:
		if addr == nil {
	...
	case *ravenutil.AddressScriptHash:
		if addr == nil {
			return nil, scriptError(ErrUnsupportedAddress,
				nilAddrErrStr)
		}
...
func PayToAddrReplayOutScript(addr ravenutil.Address, blockHash []byte, blockHeight int64) ([]byte, error) {
	const nilAddrErrStr = "unable to generate payment script for nil address"
	switch addr := addr.(type) {
	case *ravenutil.AddressPubKeyHash:
		if addr == nil {
...
func MultiSigScript(pubkeys []*ravenutil.AddressPubKey, nrequired int) ([]byte, error) {
...
func ExtractPkScriptAddrs(pkScript []byte, chainParams *chaincfg.Params) (ScriptClass, []ravenutil.Address, int, error) {
	var addrs []ravenutil.Address
	...
	case PubKeyHashTy, PubKeyHashReplayOutTy:
		...
		requiredSigs = 1
		addr, err := ravenutil.NewAddressPubKeyHash(pops[2].data,
			chainParams)
		if err == nil {
	...
	case PubKeyTy:
		...
		requiredSigs = 1
		addr, err := ravenutil.NewAddressPubKey(pops[0].data, chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	case ScriptHashTy, ScriptHashReplayOutTy:
		...
		requiredSigs = 1
		addr, err := ravenutil.NewAddressScriptHashFromHash(pops[1].data,
			chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	case MultiSigTy:
		...
		addrs = make([]ravenutil.Address, 0, numPubKeys)
		for i := 0; i < numPubKeys; i++ {
			addr, err := ravenutil.NewAddressPubKey(pops[i+1].data,
				chainParams)
			if err == nil {
				addrs = append(addrs, addr)
			}
