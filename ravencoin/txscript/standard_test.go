...
// newAddressPubKey returns a new ravenutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) ravenutil.Address {
	addr, err := ravenutil.NewAddressPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new ravenutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) ravenutil.Address {
	addr, err := ravenutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// newAddressScriptHash returns a new ravenutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) ravenutil.Address {
	addr, err := ravenutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the ravenutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the ravenutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := ravenutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := ravenutil.NewAddressScriptHashFromHash(hexToBytes("e8c300"+
	...
	p2pkCompressedMain, err := ravenutil.NewAddressPubKey(hexToBytes("02192d"+
	...
	p2pkCompressed2Main, err := ravenutil.NewAddressPubKey(hexToBytes("03b0b"+
	...
	p2pkUncompressedMain, err := ravenutil.NewAddressPubKey(hexToBytes("0411"+
	...
	tests := []struct {
		...
		{(*ravenutil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*ravenutil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*ravenutil.AddressPubKey)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	tests := []struct {
		keys      []*ravenutil.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*ravenutil.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
