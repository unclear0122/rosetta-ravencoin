...
GO_PACKAGES=./services/... ./indexer/... ./ravencoin/... ./configuration/...
...
build-release:
	...
	docker build -t rosetta-ravencoin:$(version) .;
	docker save rosetta-ravencoin:$(version) | gzip > rosetta-ravencoin-$(version).tar.gz;

run-mainnet-online:
	docker run -d --rm --ulimit "nofile=${NOFILE}:${NOFILE}" -v "${PWD}/ravencoin-data:/data" -e "MODE=ONLINE" -e "NETWORK=MAINNET" -e "PORT=8080" -p 8080:8080 -p 8766:8766 rosetta-ravencoin:latest

run-mainnet-offline:
	docker run -d --rm -e "MODE=OFFLINE" -e "NETWORK=MAINNET" -e "PORT=8081" -p 8081:8081 rosetta-ravencoin:latest

run-testnet-online:
	docker run -d --rm --ulimit "nofile=${NOFILE}:${NOFILE}" -v "${PWD}/ravencoin-data:/data" -e "MODE=ONLINE" -e "NETWORK=TESTNET" -e "PORT=8080" -p 8080:8080 -p 18766:18766 rosetta-ravencoin:latest
