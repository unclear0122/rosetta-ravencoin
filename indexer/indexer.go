...
type Client interface {
	...
	GetRawBlock(context.Context, *types.PartialBlockIdentifier) (*ravencoin.Block, []string, error)
	ParseBlock(
		context.Context,
		*ravencoin.Block,
...
// waitForNode returns once ravend is ready to serve
// block queries.
func (i *Indexer) waitForNode(ctx context.Context) error {
	...
	for {
		...
		logger.Infow("waiting for ravend...")
...
// Sync attempts to index Ravencoin blocks using
// the ravencoin.Client until stopped.
...
// Prune attempts to prune blocks in ravend every
// pruneFrequency.
func (i *Indexer) Prune(ctx context.Context) error {
	...
	for {
		...
		case <-tc.C:
			...
			// Must meet pruning conditions in ravencoin core
			// Source:
			// https://github.com/ravencoin/ravencoin/blob/a63a26f042134fa80356860c109edb25ac567552/src/rpc/blockchain.cpp#L953-L960
			...
			logger.Infow("attempting to prune ravend", "prune height", pruneHeight)
			prunedHeight, err := i.client.PruneBlockchain(ctx, pruneHeight)
			if err != nil {
				logger.Warnw(
					"unable to prune ravend",
					"prune height", pruneHeight,
					"error", err,
				)
			} else {
				logger.Infow("pruned ravend", "prune height", prunedHeight)
			}
		}
	}
}
...
func (i *Indexer) findCoin(
	...
	for ctx.Err() == nil {
		...
		// multiple listeners)
		transactionHash := ravencoin.TransactionHash(coinIdentifier)
...
func (i *Indexer) findCoins(
	...
	for _, coinIdentifier := range remainingCoins {
		// Wait on Channel
		txHash := ravencoin.TransactionHash(coinIdentifier)
...
func (i *Indexer) GetScriptPubKeys(
	...
	scripts := make([]*ravencoin.ScriptPubKey, len(coins))
	for j, coin := range coins {
		coinIdentifier := coin.CoinIdentifier
		transactionHash, networkIndex, err := ravencoin.ParseCoinIdentifier(coinIdentifier)
		if err != nil {
		...
		for _, op := range transaction.Operations {
			if op.Type != ravencoin.OutputOpType {
				continue
			}
			...
			var opMetadata ravencoin.OperationMetadata
