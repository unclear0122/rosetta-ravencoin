...
usage() {
  this=$1
  cat <<EOF
$this: download pre-compiled Docker images for RavenProject/rosetta-ravencoin
...
execute() {
  ...
  docker tag "rosetta-ravencoin:${TAG}" "rosetta-ravencoin:latest"
  log_info "loaded rosetta-ravencoin:${TAG} and tagged as rosetta-ravencoin:latest"
  rm -rf "${tmpdir}"
...
BINARY=rosetta-ravencoin
FORMAT=tar.gz
OWNER=RavenProject
REPO="rosetta-ravencoin"
PREFIX="$OWNER/$REPO"
